...
- **Whether a limit will last.** Claude Code and Codex cards now say "expected to last the window" under each limit, or roughly when it runs out. The time only appears when it falls before the reset — the rest of the time there is nothing to predict — and it is rounded, because usage comes in bursts and a figure to the minute would be made up.
