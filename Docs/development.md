...
- New `Provider` case: **new providers are profiled** — a `ProviderProfile` in its own file under `Providers/Profiled/`, plus one line in `ProfiledProviders.profile`, and nothing else. Every switch that used to need a case-by-case answer for a profiled provider instead switches over `Provider.HandWrittenProvider` (`UsageProvider.swift`) — the twenty-five providers written before profiles, plus `.pulseExtension` — reached through `provider.handWritten`, which is nil for a profiled provider. A profiled provider is therefore named in none of them: not the SVG, not the service returning `ProviderUsage`, not `UsageStore.refresh` / `refresh(_:)` / `fetchAdded`, not `keepsLocalTranscripts` / `supportsLocalActivity` / `hasSourceChoice` / `supportsMultipleAccounts`, not discovery, not `monitoringAccessDescription`. `AgentActivity.root(for:)` and `UsageLedger.logFiles(for:)` return optional roots. `HandWrittenProviderCoverageTests` pins that every `Provider` case is answered by exactly one of `handWritten` or `profile`. Routes, auth and the full instructions either way: [providers/README.md](providers/README.md#adding-a-provider).
