...
With auto-collapse on, the notch rail draws **nothing** at rest: no sliver, tint, enlarged black housing, or invisible grab target. Hovering the hardware rectangle for 200ms reveals it; leaving uses the existing 320ms grace period. Switching auto-collapse off still keeps it expanded. Dragging, a held press, and the panel menu keep it open. `PanelPointerWatcher` samples the hardware rectangle even though it is above the panel window, and ignores a hidden window or an inactive Space. No second window, global event monitor, or additional permission is needed.

`NotchBerthShape` draws one continuous surface from a neck no wider than the housing through a 24 × scale point shoulder to the ordinary rail width. Its top is exactly the housing's bottom, independent of menu-bar height, closing the historically measured one-point seam. A short rail uses the smaller of its width and the housing's width. The shoulder is above the rail's existing coordinates, so ring clicks, card offsets, and provider layout are unchanged. The horizontal window budget always includes that shoulder, whether open or closed; hover and card opening never resize it. Display changes and dragging refresh the transient notch rectangle through the same placement paths as the rail.
