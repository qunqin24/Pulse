...
- **`Layout.size` is whole points** (rounded up). AppKit rounds the window frame and SwiftUI draws against the rounded one, while every hit test measures against `Layout.size`. At 280.44 the window was 281: the docked rail was drawn flush with the screen edge but its hit rect stopped 0.56pt short, so a pointer pushed against the edge was off the panel — the sliver opened on entry and the sampler shut it, in a loop. `RailGeometryTests` pins it.
...
**The card's content is masked to `UsageBubbleShape`.** Switching between cards of different heights keeps one `UsageDetailCard` and swaps its rows; the outline grows on the panel's spring, but an added row is laid out at its final place at once and used to stand outside the card for the first frames. Only the content is masked — the surface keeps its own edge, where glass draws a rim. The mask is the safety net, not the motion: an added row trails the outline briefly (`rowTransition`, opacity + 6pt drop, 0.06s wait + 0.14s) and a lost one leaves in 0.06s; the header's old title leaves in 0.06s while the new one fades straight in (`headerTransition`). **Keep all of it short**: sweeping the pointer down the rail switches cards every ~0.1s and restarts every one of these, and at 0.14s + 0.22s the card spent most of a sweep empty. The card's spring (`selectedSlot`) is 0.28s for the same reason. The header is keyed by account **and** title, so split rings swap too; percentages swap outright (`.contentTransition(.identity)`) — a cross-fade drew two over each other and a numeric roll sat mid-turn, digits blank, through a sweep.
...
With auto-collapse on, the notch rail draws no collapsed surface at rest: no sliver, enlarged black housing, or invisible grab target. When a limit has crossed the configured warning threshold and dock alert colour is enabled, `NotchAlertShape` draws only a 2pt status edge directly below and inside the hardware housing; otherwise the collapse is visually empty. Entering the hardware rectangle uses the same `PointerEntryReporter` and immediate show path as the ordinary sliver, with the existing 150ms pointer sampler also checking the target; there is no notch-specific dwell timer; leaving uses the existing 320ms grace period. Switching auto-collapse off still keeps it expanded. Dragging, a held press, and the panel menu keep it open. `PanelPointerWatcher` is unchanged; it samples the same window, which now reaches the physical screen top. No second window, global event monitor, or additional permission is needed.
...
- Auto-collapse to a 6pt sliver (`AppSettings.autoCollapse`, default on) **only while docked**, except at a physical notch where the collapsed surface is empty and an active warning may draw the separate 2pt status edge. Off the edge it stays open. The ordinary sliver **is** `DockBerthShape` at `openness` 0 (`animatableData`), not a second view. Layout stays at full rail size so the card’s geometry does not change. Rings keep tracking areas only while expanded (`isInteractive`).
- The sliver takes usage colour past the warning threshold; at a notch, `NotchAlertShape` carries the same colour directly under the housing.
