...
For a top-docked notch rail, an unpainted `PointerEntryReporter` covers exactly the physical housing rectangle and calls the existing `show()` method on entry. It does not claim clicks. The same rectangle is included in `isOverContent`, so the ordinary 150ms sampler can also open and retain the rail. There is no separate dwell timer. The hidden rail has no sliver or grab target. Once expanded, the full rectangle from the screen top to the rail's bottom counts as content. Closing a menu or releasing a held panel re-evaluates the pointer so a stationary pointer cannot leave it stuck open.
