...
`AppSettings` is `@Observable`, stored in `UserDefaults`. `onChange` carries an explicit `Change`: appearance, visibility, account membership, account-specific reading inputs, refresh interval, or network proxy. AppKit re-places the panel for appearance, visibility and membership changes. `UsageStore` ignores appearance, refreshes only newly enabled accounts or accounts whose inputs changed, and reschedules an interval change without forcing a request. Hiding the panel stops activity monitoring and reschedules; showing it refreshes the readings. Proxy changes restart the helper and refresh all enabled accounts. `hidesMenuBarIcon` is the exception: its dedicated callback removes or restores the AppKit status item without refetching providers. It defaults to `false` to preserve the existing menu bar entry point.
...
- `providerOrder` / `orderedAccounts`: never trust the stored list as written. Drop unknown names; append accounts the list does not mention **in name order** after whatever arrangement is stored. The settings sidebar follows the same order. Reorder is observed directly by SwiftUI; it needs neither an AppKit frame change nor a provider request.
...
- `networkProxy` is one persisted value rather than four independently firing fields. It configures Pulse's external sessions and supported helper processes, then `onChange(.networkProxy)` queues a full refresh. Scope and the Sparkle exception: [networking.md](networking.md).
