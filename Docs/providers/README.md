...
**New providers are profiled.** Write the provider as a `ProviderProfile` in its own file, `Sources/Pulse/Providers/Profiled/<Name>UsageService.swift`. That profile carries the provider's name, icon, credential kind, access sentence, key subtitle, route, balance and pacing flags, colour, setup slug, discovery hints and fetch. Then add the case to `Provider`, one line to `Provider.profile` in `Profiled/ProfiledProviders.swift`, and the case to the profiled arm of each exhaustive switch; the compiler lists them. After that, every shared switch hands the question to the profile, and `UsageStore` asks profiled providers side by side in one task group. [`Profiled/ClinePassUsageService.swift`](../../Sources/Pulse/Providers/Profiled/ClinePassUsageService.swift) is the reference.
