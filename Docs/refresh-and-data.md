...
Settings changes carry their scope. Appearance never reads credentials or enters the refresh queue. Enabling accounts refreshes only those accounts; disabling or renaming one does not refresh its neighbours. Source, gateway, DeepSeek and Qoder settings refresh the affected enabled account, using the existing per-account queue when a pass is running. Interval changes only reschedule the timer. Hiding the panel stops the activity monitor and reschedules; showing it asks for fresh readings. Proxy changes restart the helper before a full refresh. Notification controls retain their separate authorization and reconsideration path.
...
**`refresh(dueOnly:)` is true for exactly one caller — the timer.** Every other route into a pass is *something happening*: a reading input changed, a window reset, the display woke, the app server pushed, the pointer arrived at a rail whose figures are older than the cadence allows. Each of those is a reason to look now, whatever the cadence says. Getting it backwards is not a slow refresh but **a control that does nothing**: switching DeepSeek between "since top-up" and "balance only" changes what the reading means, and a pass that skipped the provider because it had been asked a minute ago left the old ring on screen. `providersToAsk` is `nonisolated static` and pure so that rule is pinned by `RefreshPacingTests` rather than by hand.
