...
**One timer, but not one cadence.** Under `.automatic` each provider has its own interval and the timer is set for whichever is due soonest; a pass then asks only the accounts that are actually due (`UsageStore.accountsToAsk`, paced from `askedAt` — *asked*, not answered, or a provider that refuses every time reads as permanently due and spins the loop). Everything not asked keeps the reading it has, because the commit loop is gated on the same set.

Primary and added accounts each keep their own request timestamp; a failed request counts, and a manual refresh advances only that account's timestamp. Added accounts are stamped when their sequential read starts, not while waiting behind another account. The timer considers every shown account, including an added-only rail. A **fixed** interval chosen in Settings applies to everything equally: somebody who picked five minutes meant five minutes.
...
**`refresh(dueOnly:)` is true for exactly one caller — the timer.** Every other route into a pass is *something happening*: a setting changed, a window reset, the display woke, the app server pushed, the pointer arrived at a rail whose figures are older than the cadence allows. Each of those is a reason to look now, whatever the cadence says. An explicit refresh requested during another pass is queued; a timer tick is not, because the running pass will schedule the next due account when it completes. Getting it backwards is not a slow refresh but **a control that does nothing**: switching DeepSeek between "since top-up" and "balance only" changes what the reading means, and a pass that skipped the provider because it had been asked a minute ago left the old ring on screen. `accountsToAsk` is `nonisolated static` and pure so that rule is pinned by `RefreshPacingTests` rather than by hand.
