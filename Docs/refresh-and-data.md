...
**One timer, but not one cadence.** Under `.automatic` each provider has its own interval and the timer is set for whichever is due soonest; a pass then asks only the accounts that are actually due (`UsageStore.providersToAsk`, paced from `askedAt` — *asked*, not answered, or a provider that refuses every time reads as permanently due and spins the loop). Everything not asked keeps the reading it has, because the commit loop is gated on the same set. A **fixed** interval chosen in Settings applies to everything equally: somebody who picked five minutes meant five minutes. **Extensions** are asked on every pass too, side by side in a task group rather than in the added accounts' one-at-a-time queue: each is a program with its own time limit (1–60 s), and one using all of it must not hold the others back. [extensions.md](extensions.md) **Added accounts are not paced this way, deliberately**: every pass asks all of them, whichever primary woke it (the `extras` comment in `UsageStore.refresh`). A PR giving each its own cadence (#62) was declined on that basis; revisit it only on a reported problem.
...
- A confirmed empty Qoder allowance (`.qoderNoCredits`), or StepFun reporting no Step Plan (`.stepFunNoPlan`), removes that account's banked reading from memory and disk. It is a successful answer, so old figures cannot stand in on the panel, next launch or `--json` export. Failed or unreadable requests still use the ordinary fallback. [providers/qoder.md](providers/qoder.md)
