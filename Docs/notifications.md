...
**A reset needs unambiguous evidence.** A few points of drift is not a reset — a rolling window (Kimi's week) slides down without anything having turned over. A clock that moves forward *without* the tank emptying is not one either: Codex's 5-hour session pushes `resetsAt` as you use it, and treating that as a refill is how the ribbons played when nothing had reset. A forty-point drop together with a new reset time is a turnover on the first sample. A forty-point drop while the stated reset time holds still has to show up on a second live reading — Codex has reported 0% used for one sample without refilling. A drop with no reset time on either side still counts on the first sample (Kimi). Ribbons and the reset notification share this rule.
